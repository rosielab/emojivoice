...
import time
import ffmpeg
from WhisperLive.whisperlive.utils import clear_screen, print_transcript, create_srt_file, resample
    ...
    def record(self):
        ...
        def on_press(key):
            ...
            if key == Key.space:
                ...
                self.frames += data
                self.audio_array = self.bytes_to_float_array(data)
                self.multicast_packet(self.audio_array.tobytes())
        def on_release(key):
            if key == Key.space:
                self.stream.stop_stream()
                self.stream.close()
